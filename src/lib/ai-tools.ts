...
export class AITools {
  async executeOperation(operation: AIToolOperation): Promise<any> {
    try {
  ...
  private async buildProject(projectId: string): Promise<any> {
  ...
  async getProjectStructure(projectId: string): Promise<any> {
    ...
    async function buildStructure(currentPath: string, obj: any) {
      ...
      for (const item of items) {
        ...
        try {
          ...
          if (stat.isDirectory()) {
            obj[item] = {};
            await buildStructure(itemPath, obj[item]);
          } else {
            obj[item] = 'file';
          }
        } catch (error) {
