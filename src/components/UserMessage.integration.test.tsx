...
describe('UserMessage Integration', () => {
  it('renders text parts with file attachments correctly', () => {
    ...
    const content = [
      ...
      { type: 'text' as const, text: 'File added to /tmp/data.csv' },
    ...
    expect(screen.getByText('added to /tmp/data.csv')).toBeInTheDocument();
  });

  it('handles single text part with file attachment', () => {
    const content = [
      { type: 'text' as const, text: 'File added to /tmp/script.js' }
    ];
    
    render(<UserMessage content={content} />);
    
    expect(screen.getByText('script.js')).toBeInTheDocument();
    expect(screen.getByText('added to /tmp/script.js')).toBeInTheDocument();
  });

  it('handles multiple file attachments in separate parts', () => {
    const content = [
      { type: 'text' as const, text: 'Here are my files:' },
      { type: 'text' as const, text: 'File added to /tmp/config.json' },
      { type: 'text' as const, text: 'File added to /tmp/styles.css' },
      { type: 'text' as const, text: 'File added to /tmp/main.js' }
    ];
