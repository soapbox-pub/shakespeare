  ...
  useEffect(() => {
    const handleLoadingChanged = (projectId: string, isLoading: boolean) => {
      ...
      if (wasLoading && !isLoading) {
        ...
        // Use window.location.pathname to get the current path at the time the event fires
        const pathParts = window.location.pathname.split('/');
        const currentId = pathParts[2] ? decodeURIComponent(pathParts[2]) : ''; // /project/{projectId} or /chat/{chatId}
        ...
        if (!isCurrentView) {
          const viewPath = session.isChat ? `/chat/${session.projectId}` : `/project/${session.projectId}`;
          toast({
