...
/** NodeFS interface for file system operations */
...
export class DotAI {
  ...
  constructor(fs: JSRuntimeFS, workingDir: string = process.cwd()) {
    this.fs = fs;
  ...
  async addToHistory(sessionName: string, message: AIMessage): Promise<void> {
    ...
    try {
      ...
      const existingContent = await this.fs.readFile(sessionFile, "utf8");
      await this.fs.writeFile(sessionFile, existingContent + "\n" + jsonLine, "utf8");
    } catch {
      // File doesn't exist, create it with the data
      await this.fs.writeFile(sessionFile, jsonLine, "utf8");
    }
  }
  ...
  async writeAiParameters(parameters: Record<string, string>): Promise<void> {
    ...
    try {
      ...
      await this.fs.writeFile(parametersPath, content, "utf8");
    } catch (error) {
